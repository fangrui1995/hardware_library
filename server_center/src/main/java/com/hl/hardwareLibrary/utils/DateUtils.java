...
public class DateUtils {
	...
	public static final String FM_DATE_AND_HOUR = "yyyy-MM-dd-HH";
	...
	public final static String FM_PATTERN_EN_YMD = "yyyyMMdd";

	public final static String FM_KQ_DATE = "yyyy-MM-dd";

	public final static String FM_KQ_DATA = "HH:mm";

	public static final String FM_KQ_AND_TIME = "yyyy-MM-dd HH:mm";
	...
	// 2002-09-23 09:30:00
	public static String getCalendarModel(Timestamp date) {
		if (date != null) {
	...
	public static Collection<String> getAllYearForDay(String yy, String mm,
		...
		if ((incol != null) && (incol.size() > 0)) {
			...
			Iterator<String> it = incol.iterator();
	...
	public static Date getDateByFormat(String format, String datetime) {
		SimpleDateFormat sdf = null;
		Date d = null;
...
// }
	...
	public static String getDateDs(String date) {
...
// System.out.println(dayNames[dayOfWeek - 1]);
	...
	public static int getWorkingDayExitSat(Calendar d1, Calendar d2) {
		int result = -1;
		if (d1.after(d2)) { // swap dates so that d1 is start and d2 is end
			Calendar swap = d1;
			d1 = d2;
		...
		stmp = 7 - d1.get(Calendar.DAY_OF_WEEK);
		etmp = 7 - d2.get(Calendar.DAY_OF_WEEK);
		if (stmp != 6) {// 开始日期为星期六和星期日时偏移量为0
			charge_start_date = stmp - 1;
		}
		if (etmp != 6) {// 结束日期为星期六和星期日时偏移量为0
			charge_end_date = etmp - 1;
		}
// }
		result = (getDaysBetween(getNextMonday(d1), getNextMonday(d2)) / 7)
	...
	public static int getWorkingDayExitSun(Calendar d1, Calendar d2) {
		int result = -1;
		if (d1.after(d2)) { // swap dates so that d1 is start and d2 is end
			Calendar swap = d1;
			d1 = d2;
		...
		stmp = 7 - d1.get(Calendar.DAY_OF_WEEK);
		etmp = 7 - d2.get(Calendar.DAY_OF_WEEK);
		if (stmp != 0) {// 开始日期为星期六和星期日时偏移量为0
			charge_start_date = stmp - 1;
		}
		if (etmp != 0) {// 结束日期为星期六和星期日时偏移量为0
			charge_end_date = etmp - 1;
		}
// }
		result = (getDaysBetween(getNextMonday(d1), getNextMonday(d2)) / 7)
	 ...
	 * @return
	 */
	public static String getDs(String date){
		DateTime endOfMonth = DateUtil.endOfMonth(DateUtil.parse(date, "yyyy-MM"));
		return DateUtil.format(endOfMonth, "yyyyMMdd");
	}


	public static String getLastMonthDs(String date){

		DateTime parse = DateUtil.parse(date, "yyyy-MM");
		DateTime offsetMonth = DateUtil.offsetMonth(parse, -1);
		DateTime endOfMonth = DateUtil.endOfMonth(offsetMonth);
		return DateUtil.format(endOfMonth, "yyyyMMdd");
	}


	public static String getLastYearDs(String date){

		DateTime parse = DateUtil.parse(date, "yyyy-MM");
		DateTime offsetYear = DateUtil.offsetMonth(parse, -12);
		DateTime endOfMonth = DateUtil.endOfMonth(offsetYear);
		return DateUtil.format(endOfMonth, "yyyyMMdd");
	}


	public static String getLastTwoMonthDs(String date){
		DateTime parse = DateUtil.parse(date, "yyyy-MM");
		DateTime offsetMonth = DateUtil.offsetMonth(parse,-2);
		DateTime endOfMonth = DateUtil.endOfMonth(offsetMonth);
		return DateUtil.format(endOfMonth, "yyyyMMdd");
	}
	 ...
	 * @return
	 */
	public static String getBeginOfYearDs(String date){
		DateTime beginOfYear = DateUtil.beginOfYear(DateUtil.parse(date, "yyyy-MM"));
	 ...
	 * @return
	 */
	public static String getLastEndOfYearDs(String ds){
		DateTime endOfYear = DateUtil.endOfYear(DateUtil.parse(ds, "yyyyMMdd"));
		DateTime dateTime = DateUtil.endOfMonth(endOfYear);
		return DateUtil.format(dateTime, "yyyyMMdd");
	}
	 ...
	 * @return
	 */
	public static Date StringToDate(String format, String datetimeStr) {
		if (StringUtils.isBlank(format) || StringUtils.isBlank(datetimeStr)) {
			return null;
		}
		...
		try {
			...
			log.info("StringToDate Exception: ", e);
		}
		return date;
	}
